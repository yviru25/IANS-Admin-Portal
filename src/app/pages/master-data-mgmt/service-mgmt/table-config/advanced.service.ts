...
import { ServiceModel, SearchResult } from './service.model';
 ...
 */
function sort(tables: ServiceModel[], column: string, direction: string): ServiceModel[] {
    if (direction === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: ServiceModel, term: string, pipe: PipeTransform) {
    return tables.serviceCode.toLowerCase().includes(term)
}
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<ServiceModel[]>([]);
    // tslint:disable-next-line: variable-name
